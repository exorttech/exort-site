(() => {
  ...
  const copy = {
    ru: {
      ...
      beforeMenu: "ПЕРЕД ПРОСМОТРОМ МЕНЮ",
      serviceTitle: "Обратите внимание",
      serviceNotice: "К счёту добавляется 10% за обслуживание.",
      allergyNotice: "Если у вас есть аллергия, пожалуйста, сообщите официанту.",
      understood: "Понятно",
    ...
    kk: {
      ...
      beforeMenu: "МӘЗІРДІ КӨРМЕС БҰРЫН",
      serviceTitle: "Назар аударыңыз",
      serviceNotice: "Шотқа 10% қызмет көрсету ақысы қосылады.",
      allergyNotice: "Аллергияңыз болса, даяшыға алдын ала хабарлаңыз.",
    ...
    en: {
      ...
      beforeMenu: "BEFORE VIEWING THE MENU",
      serviceTitle: "Please note",
      serviceNotice: "A 10% service charge is added to the bill.",
      allergyNotice: "If you have an allergy, please tell your waiter.",
      understood: "Got it",
  ...
  let serviceNoticeCloseTimer = 0;

  const els = {
    app: document.querySelector(".demo-layout"),
    ...
    serviceNoticeLayer: document.querySelector("[data-service-notice-layer]"),
    serviceNoticeModal: document.querySelector("[data-service-notice-modal]"),
    serviceNoticeName: document.querySelector("[data-service-notice-name]"),
  ...
  function applyRestaurantProfile() {
    ...
    els.serviceNoticeName.textContent = restaurant.name || "";
  ...
  function renderNav() {
    ...
    els.nav.innerHTML = [
      !state.query.trim() && state.items.length
        ? `<button type="button" class="is-active" data-nav-target="popular" style="--motion-order: 0">${escapeHtml(t("popular"))}</button>`
        : "",
      ...visible.map((category, index) => `<button type="button" data-nav-target="category-${category.id}" style="--motion-order: ${index + 1}">${escapeHtml(categoryName(category))}</button>`),
    ].join("");
  }
  ...
  function renderFeatured() {
    ...
    els.featured.innerHTML = featured.map((item, index) => `
      <button class="featured-card motion-card" type="button" data-item-id="${item.id}" aria-label="${escapeHtml(itemName(item))}" style="--motion-order: ${index}">
  ...
  function renderCard(item, index = 0) {
    ...
    return `
      <button class="menu-card motion-card${item.image_url ? "" : " menu-card--no-image"}${available ? "" : " menu-card--unavailable"}"
        type="button" data-item-id="${item.id}" aria-label="${escapeHtml(itemName(item))}" style="--motion-order: ${Math.min(index, 6)}">
  ...
  function renderSections() {
    let count = 0;
    els.sections.innerHTML = state.categories.map((category, categoryIndex) => {
      ...
      return `
        <section class="menu-section" id="category-${category.id}" data-menu-section>
          <header class="menu-section__header motion-heading" style="--motion-order: ${Math.min(categoryIndex, 4)}">
            <div><h2>${escapeHtml(categoryName(category))}</h2></div>
            <p>${items.length} ${escapeHtml(t("items"))}</p>
          </header>
          <div class="menu-list">${items.map((item, index) => renderCard(item, index)).join("")}</div>
        </section>
      `;
    }).join("");
  ...
  function observeSections() {
    ...
    sectionObserver = new IntersectionObserver(() => {
      const visible = [...sections]
        ...
        .filter(({ rect }) => rect.bottom > 72 && rect.top < window.innerHeight * 0.72)
        .sort((a, b) => {
          const aPastNav = a.rect.top <= 96;
          const bPastNav = b.rect.top <= 96;
          if (aPastNav !== bPastNav) return aPastNav ? -1 : 1;
          return aPastNav ? b.rect.top - a.rect.top : a.rect.top - b.rect.top;
        })[0];
      if (!visible) return;
      const target = visible.section.id;
  ...
  function openSheet(itemId, trigger = null, shouldTrack = true) {
    ...
    setTimeout(() => document.querySelector("[data-sheet-close].sheet-close")?.focus(), motionDuration() + 20);
  ...
  function openServiceNotice() {
    state.serviceNoticeOpen = true;
    window.clearTimeout(serviceNoticeCloseTimer);
    els.serviceNoticeLayer.classList.add("is-open");
    els.serviceNoticeLayer.setAttribute("aria-hidden", "false");
    els.app.inert = true;
    document.body.classList.add("is-service-notice-open");
    setTimeout(() => els.serviceNoticeLayer.querySelector("[data-service-notice-accept]")?.focus(), motionDuration() + 20);
  }

  function closeServiceNotice() {
    state.serviceNoticeOpen = false;
    els.serviceNoticeLayer.classList.remove("is-open");
    els.serviceNoticeLayer.setAttribute("aria-hidden", "true");
    window.clearTimeout(serviceNoticeCloseTimer);
    serviceNoticeCloseTimer = window.setTimeout(() => {
      els.app.inert = false;
      document.body.classList.remove("is-service-notice-open");
      document.querySelector("[data-scroll-menu]")?.focus();
    }, motionDuration());
  }
  ...
  function setupStickyState() {
    if (!els.heroSection || !("IntersectionObserver" in window)) return;
    const observer = new IntersectionObserver(([entry]) => {
      const isPastHero = !entry.isIntersecting && entry.boundingClientRect.bottom <= 0;
      els.nav.classList.toggle("is-stuck", isPastHero);
    }, { threshold: 0 });
    observer.observe(els.heroSection);
  }

  function trapFocus(event) {
    if (event.key !== "Tab") return;
    const modal = state.serviceNoticeOpen
      ? els.serviceNoticeModal
      : state.infoOpen
        ? els.infoModal
        : state.sheetItem
          ? els.sheet
  ...
  document.addEventListener("click", (event) => {
    if (event.target.closest("[data-service-notice-accept]")) return closeServiceNotice();
  ...
  document.addEventListener("keydown", (event) => {
    if (event.key === "Escape") {
      if (state.serviceNoticeOpen) closeServiceNotice();
      else if (state.infoOpen) closeInfo();
  ...
  async function loadMenu() {
    try {
      ...
      requestAnimationFrame(openServiceNotice);
      startAnalytics();
    } catch (error) {
      console.error("[public-menu] Failed to load restaurant data:", error);
      document.body.classList.add("is-menu-ready");
      els.empty.hidden = false;
      const title = els.empty.querySelector("h2");
      const description = els.empty.querySelector("p");
      const resetButton = els.empty.querySelector("[data-reset-search]");
