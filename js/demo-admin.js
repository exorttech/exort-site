...
function ensureAdminEnhancements() {
  ...
  if (langTabs && !document.querySelector("[data-translate-current-item]")) {
    ...
    tools.innerHTML = `
      ...
      <small>Перевод идет через backend. API-ключи не хранятся во frontend.</small>
    `;
    langTabs.insertAdjacentElement("afterend", tools);
  }

  const formGrid = el.itemForm?.querySelector(".form-grid");
  if (formGrid && !el.itemForm.elements.is_temporarily_unavailable) {
    formGrid.insertAdjacentHTML("beforeend", `
      <label class="check-line">
        <input name="is_temporarily_unavailable" type="checkbox" />
        Временно недоступно
      </label>
      <label>
        Недоступно до
        <input name="inactive_until" type="datetime-local" />
      </label>
    `);
  }
  ...
  if (imageEditor && !document.querySelector("[data-remove-editor-image]")) {
    imageEditor.insertAdjacentHTML("beforeend", `
      <div class="media-actions">
        ...
        <small>Фото будет сжато и загружено в Supabase Storage.</small>
      </div>
    `);
  }
...
function bindEvents() {
  ...
  el.itemForm.addEventListener("input", () => { state.dirty = true; });
...
function showLoginError(message) {
  el.loginError.textContent = message.includes("Failed to fetch")
    ? "Backend админки недоступен. Проверьте Netlify Function и переменные Supabase."
    : message;
}
...
function renderMetrics() {
  ...
  const stopped = state.items.filter((item) => item.is_stoplisted).length;
  const values = [
    ["Всего блюд", state.items.length, "В каталоге"],
    ["Активные блюда", active, "Доступны гостям"],
    ["В стоп-листе", stopped, "Показываются с бейджем"],
    ["Категории", state.categories.length, "Разделов меню"],
  ];
...
function renderAttention() {
  const issues = [
    ...
    { name: "Стоп-лист", count: state.items.filter((item) => item.is_stoplisted).length, view: "stoplist", type: "neutral" },
...
function filteredItems() {
  ...
  return state.items.filter((item) => {
    ...
    const matchesStatus =
      ...
      (status === "stop" && item.is_stoplisted) ||
...
function renderDishes() {
  ...
  el.dishGrid.innerHTML = items.length ? items.map((item) => {
    ...
    return `<article class="dish-card ${statusClass !== "active" ? "is-muted" : ""}">
      <div class="dish-visual">${visual(item)}<span class="dish-status ${statusClass}">${statusText}</span></div>
      <div class="dish-body">
        ...
        <div class="dish-meta"><span>${escapeHtml(categoryName(item.category_id))}</span><button class="stop-switch ${item.is_stoplisted ? "" : "is-on"}" type="button" data-toggle-stock="${item.id}" aria-label="Изменить стоп-лист"></button></div>
...
function renderStopList() {
  el.stopList.innerHTML = state.items.map((item) => `<article class="stop-row">
    ...
    <span class="stop-state ${item.is_stoplisted ? "" : "is-sale"}">${item.is_stoplisted ? "Стоп" : "В продаже"}</span>
    <button class="large-switch ${item.is_stoplisted ? "" : "is-on"}" type="button" data-toggle-stock="${item.id}" aria-label="Изменить доступность"></button>
  </article>`).join("");
}
...
function openItemDrawer(id = "") {
  ...
  el.itemForm.elements.is_temporarily_unavailable.checked = isTemporarilyUnavailable(item || {});
  el.itemForm.elements.inactive_until.value = item?.inactive_until ? toDatetimeLocal(item.inactive_until) : "";
...
async function handleItemSubmit(event) {
  ...
  const inactiveUntil = el.itemForm.elements.is_temporarily_unavailable.checked
    ? (data.inactive_until ? new Date(data.inactive_until).toISOString() : getNextMidnightIso())
    : null;
  const payload = {
    id: existing?.id || "",
    ...
    is_active: el.itemForm.elements.is_active.checked,
    is_stoplisted: existing?.is_stoplisted || false,
...
async function handleEditorFile() {
  try {
    ...
    el.editorImage.innerHTML = `<img src="${imageData}" alt="" />`;
    state.dirty = true;
  } catch (error) {
...
function applyTheme(theme) {
  ...
  el.themeToggles.forEach((toggle) => toggle.setAttribute("aria-pressed", String(nextTheme === "dark")));
