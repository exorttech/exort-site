(function () {
  const state = { root: null, data: null, range: "today", heatmapRange: "current_week", sourceId: "all", metric: "sessions", dishTab: "leaders", customStart: "", customEnd: "", loading: false, error: "", mounted: false, requestId: 0, detailOpen: false, dishDetailOpen: false, eventsDetailOpen: false, timelineSort: { key: "sortValue", direction: "asc" }, dishSort: { key: "default", direction: "asc" } };
  ...
  function activityCard(days) {
    ...
    return `<section class="analytics-v2-card analytics-activity-card">
      <div class="analytics-v2-card-head"><div><p class="kicker">Главный график</p><h3>Активность гостей за ${state.range === "7d" || state.range === "prev_week" ? "неделю" : "период"}</h3></div>
  ...
  function onClick(event) {
    ...
    if (period) { state.range = period; state.timelineSort = { key: "sortValue", direction: "asc" }; state.dishSort = { key: "default", direction: "asc" }; if (period !== "custom") load(); else render(); return; }
  ...
  window.ExortAnalytics = { mount, reload: load, closeDetail };
})();
